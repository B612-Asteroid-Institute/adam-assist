...
mod tests {
    ...
    fn live_assist_matches_public_semantics_fixture_ephemeris_case() {
        ...
        for case in cases {
            ...
            // The package-level Rust path agrees tightly in range/angles
            // (<=1.4e-10) and has a stable 2.35e-8 floor in spherical velocity
            // components relative to the pinned legacy Python composition.
            assert!(
                max_abs < 3.0e-8,
                "{case_id} ephemeris spherical residual {max_abs:e} exceeds parity tolerance"
            );
        }
    }
